...
func Deploy(mode, target, path string, docker bool, ldFlags, tags string, fast bool, device string, vagrant bool, vagrantsystem string, comply bool, useuic bool, quickcompiler bool) {
	...
	case "build", "test":
		...
		if utils.UseGOMOD(path) {
			...
			if utils.QT_DOCKER() {
				cmd := exec.Command("go", "get", "-v", "-d", "github.com/ilius/qt/internal/binding/files/docs/"+utils.QT_API(utils.QT_VERSION())+"@"+cmd.QtModVersion(filepath.Dir(utils.GOMOD(path)))) //TODO: needs to pull 5.8.0 if QT_WEBKIT
				cmd.Dir = path
