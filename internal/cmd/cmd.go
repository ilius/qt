...
func ParseFlags() bool {
	...
	if api := utils.QT_API(""); api != "" && err == nil && !utils.QT_DOCKER() && !forDocker {
		if utils.GoListOptional("{{.Dir}}", "github.com/ilius/qt/internal/binding/files/docs/"+api, "-find", "get doc dir") == "" {
			...
			if !utils.UseGOMOD("") {
				if o := utils.GoListOptional("{{join .Imports \"|\"}}", "github.com/ilius/qt/internal/binding/files/docs", "get doc dir"); o != "" {
					for _, v := range strings.Split(o, "|") {
						fmt.Println(strings.TrimPrefix(strings.TrimSpace(strings.Replace(v, "'", "", -1)), "github.com/ilius/qt/internal/binding/files/docs/"))
					}
				}
			} else {
				...
				for mid := 5; mid <= 15; mid++ {
					for min := 0; min <= 5; min++ {
						...
						go func(mid, min int) {
							...
							if utils.GoListOptional("{{.Dir}}", "github.com/ilius/qt/internal/binding/files/docs/"+v, "-find", "get doc dir") != "" {
								fmt.Println(v)
							}
							wg.Done()
						}(mid, min)
...
func virtual(arg []string, target, path string, writeCacheToHost bool, docker bool, system string) {
	...
	if utils.UseGOMOD(path) {
		...
		if writeCacheToHost {
			...
			if strings.Contains(path, "github.com/ilius/qt/internal/examples") && !strings.Contains(path, "github.com/ilius/qt/internal/examples/androidextras") {
				gpath += pathseperator + gpfs
			} else {
